...
# Auto-install if distro is missing
ROOTFS_DIR="/data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/\$PROOT_DISTRO"
if [ ! -d "\$ROOTFS_DIR" ]; then
    echo "[!] \$PROOT_LABEL is not installed."
    read -p "    Install now? (Y/n): " _INSTALL_ANS
    _INSTALL_ANS=\${_INSTALL_ANS:-Y}
    if [[ "\$_INSTALL_ANS" =~ ^[Yy] ]]; then
        echo "[*] Installing \$PROOT_LABEL... (this may take a while)"
